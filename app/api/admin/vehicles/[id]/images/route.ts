...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (primaryImage instanceof File) {
      const optimizedPrimary = await optimizeImageForUpload(primaryImage);
      const inserted = await insertVehicleImage(
        vehicleId,
        optimizedPrimary.buffer,
        optimizedPrimary.fileName,
        true,
        optimizedPrimary.contentType
      );
      insertedImages.push(inserted);
    }

    for (const image of validExtraImages) {
      const optimizedExtra = await optimizeImageForUpload(image);
      const inserted = await insertVehicleImage(
        vehicleId,
        optimizedExtra.buffer,
        optimizedExtra.fileName,
        false,
        optimizedExtra.contentType
      );
      insertedImages.push(inserted);
    }
